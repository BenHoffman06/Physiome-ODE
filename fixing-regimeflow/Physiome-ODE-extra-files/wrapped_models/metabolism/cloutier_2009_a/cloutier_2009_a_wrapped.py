...
def createLegends():
    ...
    legend_algebraic[5] = "V_pk in component V_pk (mM_per_s)"
    legend_states[3] = "PYR in component PYR (mM)"
    legend_algebraic[1] = "V_ldh in component V_ldh (mM_per_s)"
    legend_algebraic[6] = "V_op in component V_op (mM_per_s)"
    ...
    legend_algebraic[7] = "V_ck in component V_ck (mM_per_s)"
    legend_algebraic[8] = "dAMP_dATP in component dAMP_dATP (dimensionless)"
    ...
    legend_algebraic[10] = "ATP_inh in component ATP_inh (dimensionless)"
    ...
    legend_algebraic[9] = "AMP in component AMP (mM)"
...
def computeRates(voi, states, constants):
    ...
    algebraic[5] = constants[12]*(states[2]/(states[2]+constants[14]))*(algebraic[4]/(algebraic[4]+constants[13]))*0.110400
    algebraic[1] = constants[18]*states[3]-constants[19]*states[4]
    algebraic[6] = constants[15]*(states[3]/(states[3]+constants[17]))*(algebraic[4]/(algebraic[4]+constants[16]))*(1.00000/(1.00000+0.100000*24.0000))
    rates[3] = algebraic[5]-(algebraic[6]+algebraic[1])
    algebraic[2] = constants[22]-states[6]
    algebraic[7] = constants[20]*states[6]*algebraic[4]-constants[21]*algebraic[2]*states[5]
    rates[6] = -algebraic[7]
    algebraic[9] = constants[28]-(states[5]+algebraic[4])
    algebraic[11] = power((1.00000+algebraic[9]/constants[33])/(1.00000+constants[32]*(algebraic[9]/constants[33])), 4.00000)
    algebraic[10] = power((1.00000+constants[30]*(states[5]/constants[31]))/(1.00000+states[5]/constants[31]), 4.00000)
    algebraic[12] = constants[3]*(states[0]/(states[0]+constants[5]))*(states[5]/(states[5]+constants[4]))*(states[1]/(states[1]+constants[6]))*algebraic[10]*algebraic[11]
    rates[2] = 2.00000*algebraic[12]-algebraic[5]
    ...
    algebraic[17] = (power(algebraic[9]/constants[40], constants[41]))/(1.00000+power(algebraic[9]/constants[40], constants[41]))
    ...
    algebraic[8] = -1.00000+constants[29]/2.00000+-(0.500000*(power(algebraic[3], 1.0/2)))+constants[29]*(constants[28]/(states[5]*(power(algebraic[3], 1.0/2))))
    rates[5] = ((2.00000*algebraic[5]+15.0000*algebraic[6]+algebraic[7])-(algebraic[0]+algebraic[12]+algebraic[18]+algebraic[15]))*(power(1.00000-algebraic[8], -1.00000))
    return(rates)
...
class Config:
    def __init__(
        ...
        self.model_name = "cloutier_2009_a"
        self.curve_names = [
            "F6P",
            "F26P",
            "GAP",
        ...
        self.algebraic_names = ['V_hk', 'V_ldh', 'Cr', 'u', 'ADP', 'V_pk', 'V_op', 'V_ck', 'dAMP_dATP', 'AMP', 'ATP_inh', 'AMP_act', 'V_pfk', 'unitpulseSB', 'v_stim', 'V_ATPase', 'V_lac', 'AMP_pfk2', 'V_pfk2']
        self.params = param
        self.T = T
        self.T_unit = T_unit
