...
class Parameters:
    def __init__(self):
        ...
        self.K_mNai = 10
        self.K_mKo = 1.5
        self.K_m_ns_Ca = 0.0012
        self.c1 = 0.00025
        self.c2 = 0.0001
        self.gamma = 0.15
        self.G_rel_max = 60
        self.G_rel_overload = 4
        self.tau_tr = 180
        self.K_mrel = 0.0008
        self.delta_Ca_ith = 0.00018
        self.CSQN_max = 10
        self.K_mCSQN = 0.8
        self.K_mup = 0.00092
        self.I_up = 0.00875
        ...
        self.CMDN_max = 0.05
        self.CSQNthresh = 0.7
        self.Logicthresh = 0.98
        self.preplength = 0.1
        self.radius = 0.011
        self.g_to = 0.00000*0.500000
        self.P_ns_Ca = 1.75000e-07
    ...
    def to_dict(self):
        return {
            ...
            "nKNa": self.nKNa,
            "kdKNa": self.kdKNa,
            "i_K_ATP_on": self.i_K_ATP_on,
            "nATP": self.nATP,
            "nicholsarea": self.nicholsarea,
            "ATPi": self.ATPi,
            "hATP": self.hATP,
            "kATP": self.kATP,
            "K_mpCa": self.K_mpCa,
            ...
            "c2": self.c2,
            "gamma": self.gamma,
            "G_rel_max": self.G_rel_max,
            "G_rel_overload": self.G_rel_overload,
            "tau_tr": self.tau_tr,
            "K_mrel": self.K_mrel,
            "delta_Ca_ith": self.delta_Ca_ith,
            "CSQN_max": self.CSQN_max,
            "K_mCSQN": self.K_mCSQN,
            "K_mup": self.K_mup,
            "I_up": self.I_up,
            "Ca_NSR_max": self.Ca_NSR_max,
            "A_cap": self.A_cap,
            "K_mTn": self.K_mTn,
            "K_mCMDN": self.K_mCMDN,
            "Tn_max": self.Tn_max,
            "CMDN_max": self.CMDN_max,
            "CSQNthresh": self.CSQNthresh,
            "Logicthresh": self.Logicthresh,
            "preplength": self.preplength,
            "radius": self.radius,
            "g_to": self.g_to,
...
class Config:
    def __init__(
        ...
        T: int = 100,
        T_unit: float = 0.01,
        y0=[-85.2119207874627, 13.3649235394859, 0.00149183115674257, 0.985596581239651, 0.990898461370389, 0.000117482029668194, 141.056872392446, 5.82597094505446e-6, 0.997765362821995, 0.00136737866785149, 0.98881442877378, 0.000204700363126417, 0.00660746743356887, 0.0303768241233812, 0.0144622472219576, 0.999945568566232, 1.12791401197882, 1.76731003671612, 9.65910542308504e-196, 5.33944967562997e-195, 0.000129515197402902, 2.69380318286645e-196, 0, 0, 0],
    ):
        if param is None:
            raise ValueError("Config requires param=Parameters()")

        self.model_name = "faber_rudy_2000"
        self.curve_names = [
            "V",
            "Nai",
            "m",
        ...
        self.state_names = ['V', 'Nai', 'm', 'h', 'j', 'Cai', 'Ki', 'd', 'f', 'b', 'g', 'xr', 'xs1', 'xs2', 'zdv', 'ydv', 'Ca_JSR', 'Ca_NSR', 'APtrack', 'APtrack2', 'APtrack3', 'Cainfluxtrack', 'OVRLDtrack', 'OVRLDtrack2', 'OVRLDtrack3']
        self.algebraic_names = ['E0_m', 'alpha_h', 'alpha_j', 'I_st', 'E0_d', 'f_infinity', 'b_inf', 'g_inf', 'xr_infinity', 'xs1_infinity', 'xs2_infinity', 'alpha_zdv', 'alpha_ydv', 'G_rel_Viswanathan', 'alpha_m', 'beta_h', 'beta_j', 'E_Na', 'd_infinity', 'tau_f', 'tau_b', 'tau_g', 'tau_xr', 'tau_xs1', 'tau_xs2', 'beta_zdv', 'beta_ydv', 'beta_m', 'i_Na', 'tau_d', 'alpha_f', 'tau_zdv', 'tau_ydv', 'I_CaCa', 'alpha_d', 'beta_f', 'zdv_ss', 'ydv_ss', 'I_CaNa', 'beta_d', 'I_CaK', 'f_Ca', 'i_CaCa', 'i_CaNa', 'i_CaK', 'i_Ca_L', 'Rect', 'E_Ks', 'g_Ks', 'i_Ks', 'E_K', 'i_Kr', 'alpha_K1', 'beta_K1', 'K1_infinity', 'i_K1', 'Kp', 'i_Kp', 'pona', 'pov', 'i_K_Na', 'i_K_ATP', 'rvdv', 'i_to', 'i_p_Ca', 'i_Na_b', 'E_Ca', 'i_Ca_T', 'i_Ca_b', 'f_NaK', 'i_NaK', 'I_ns_Na', 'I_ns_K', 'i_ns_Na', 'i_ns_K', 'i_ns_Ca', 'i_NaCa', 'dVdt', 'RyRopen', 'RyRclose', 'G_rel', 'i_rel', 'i_up', 'i_leak', 'i_tr']
        self.params = param
        self.T = T
        self.T_unit = T_unit
    ...
    def _build_constants(self):
        ...
        # derived constants
        c[68] = pi*c[66]*(power(c[67], 2.00000))
        c[69] = c[24]*(power(c[21]/5.40000, 1.0/2))
        c[70] = c[27]*(power(c[21]/5.40000, 1.0/2))
        c[71] = (c[32]*0.000193000)/c[34]
        c[73] = (1.00000/7.00000)*(exp(c[9]/67.3000)-1.00000)
        c[75] = c[57]/c[58]
        c[76] = 0.680000*c[68]
        c[77] = 1.00000/(1.00000+power(c[35]/c[37], c[36]))
        c[78] = 0.00480000*c[68]
        c[79] = c[71]*c[77]*(power(c[21]/4.00000, c[33]))
        c[80] = 0.0552000*c[68]
