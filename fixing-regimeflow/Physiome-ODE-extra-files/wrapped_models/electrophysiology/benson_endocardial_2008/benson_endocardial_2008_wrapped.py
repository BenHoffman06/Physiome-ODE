...
def initConsts():
    ...
    constants[54] = 1.124
    constants[55] = 0.0087
    constants[56] = 1000.00* pi*constants[8]*constants[8]*constants[7]
    constants[57] = custom_piecewise([equal(constants[6] , 0.00000), 11.5000 , True, 4.00000*11.5000])
    constants[58] = 0.0138542*(power(constants[1]/5.40000, 1.0/2))
    constants[59] = (exp(constants[3]/67.3000)-1.00000)/7.00000
    constants[60] = 2.00000* pi*constants[8]*constants[8]+2.00000* pi*constants[8]*constants[7]
...
def computeRates(voi, states, constants):
    ...
    algebraic[12] = 0.00260000/(1.00000+exp((states[0]+61.0000)/5.00000))
    ...
    algebraic[52] = (constants[36]*(1.00000-states[25]))/(1.00000+constants[37]/states[4])
    ...
    algebraic[71] = ((algebraic[69]+1.00000)*constants[41]*states[12])/((states[12]+constants[42])-algebraic[67])
    algebraic[73] = (constants[41]*states[28])/constants[43]
    algebraic[75] = (states[28]-states[24])/120.000
    rates[28] = (algebraic[71]-(algebraic[75]*constants[66])/constants[65])-algebraic[73]
    ...
    algebraic[43] = 1.00000/(1.00000+power(constants[20]/algebraic[42], 2.00000))
    algebraic[44] = (power(states[18], 3.00000))*constants[2]*exp(constants[23]*states[0]*constants[5])-(power(constants[3], 3.00000))*algebraic[42]*exp((constants[23]-1.00000)*states[0]*constants[5])
    algebraic[45] = 1.00000+constants[22]*exp((constants[23]-1.00000)*states[0]*constants[5])
    algebraic[46] = constants[27]*(power(states[18], 3.00000))+(power(constants[25], 3.00000))*algebraic[42]+(power(constants[24], 3.00000))*constants[2]*(1.00000+algebraic[42]/constants[26])
    algebraic[47] = constants[26]*(power(constants[3], 3.00000))*(1.00000+power(states[18]/constants[24], 3.00000))+(power(states[18], 3.00000))*constants[2]+(power(constants[3], 3.00000))*algebraic[42]
    algebraic[48] = (constants[21]*algebraic[43]*algebraic[44])/(algebraic[45]*(algebraic[46]+algebraic[47]))
    algebraic[76] = (states[4]-states[12])/0.200000
    algebraic[79] = 1.00000/(1.00000+(constants[49]*constants[47])/(power(states[12]+constants[47], 2.00000))+(constants[46]*constants[48])/(power(states[12]+constants[46], 2.00000)))
    rates[12] = algebraic[79]*((-((algebraic[50]+algebraic[49])-2.00000*algebraic[48])*constants[68])/(constants[64]*2.00000)+((algebraic[73]-algebraic[71])*constants[65])/constants[64]+(algebraic[76]*constants[67])/constants[64])
    ...
    algebraic[80] = 1.00000/(1.00000+(constants[52]*constants[53])/(power(constants[53]+states[4], 2.00000))+(constants[54]*constants[55])/(power(constants[55]+states[4], 2.00000)))
    rates[4] = algebraic[80]*(((-algebraic[31]*constants[68])/(constants[67]*2.00000)+(algebraic[64]*constants[66])/constants[67])-algebraic[76])
    algebraic[78] = 1.00000/(1.00000+(constants[50]*constants[51])/(power(states[24]+constants[50], 2.00000)))
    ...
    algebraic[29] = constants[57]*states[2]*states[2]*states[2]*states[1]*states[3]
    algebraic[62] = algebraic[29]*(states[0]-algebraic[59])
    algebraic[40] = 1.00000/(1.00000+0.124500*exp(-0.100000*states[0]*constants[5])+0.0365000*constants[59]*exp(-states[0]*constants[5]))
    algebraic[41] = (((constants[19]*algebraic[40]*1.00000)/(1.00000+power(constants[17]/states[18], 2.00000)))*constants[1])/(constants[1]+constants[18])
    algebraic[65] = constants[33]*(power(states[21], 3.00000))*states[22]*(states[0]-algebraic[59])
    ...
    algebraic[38] = 0.0224077*(1.00000+0.600000/(1.00000+power(3.80000e-05/states[12], 1.40000)))
    algebraic[84] = log((constants[1]+constants[34]*constants[3])/(states[23]+constants[34]*states[18]))/constants[5]
    ...
    algebraic[81] = constants[58]*states[11]*algebraic[37]*(states[0]-algebraic[70])
    algebraic[39] = exp(states[0]/300.000)
    algebraic[82] = constants[16]*(power(states[15], 3.00000))*states[16]*states[17]*algebraic[39]*(states[0]-algebraic[70])
    ...
    algebraic[53] = (constants[30]*states[0]*constants[0]*constants[5]*(states[19]-constants[4]*exp(states[0]*constants[5])))/(1.00000-exp(states[0]*constants[5]))
    ...
    algebraic[90] = (constants[44]*(power(algebraic[59]-algebraic[87], 4.00000)))/(power(algebraic[59]-algebraic[87], 4.00000)+power(87.8251, 4.00000))
    rates[18] = (-algebraic[68]*constants[68])/constants[64]+algebraic[90]
    algebraic[91] = (constants[45]*(algebraic[70]-algebraic[87]))/((algebraic[70]-algebraic[87])+87.8251)
    rates[23] = (-algebraic[86]*constants[68])/constants[64]+algebraic[91]
    rates[19] = (algebraic[89]*constants[68])/constants[64]+algebraic[90]+algebraic[91]
    return(rates)
...
class Config:
    def __init__(
        ...
        self.model_name = "benson_endocardial_2008"
        self.curve_names = [
            "V",
            "H",
            "m",
    ...
    def _build_constants(self):
        ...
        # derived constants
        c[56] = 1000.00* pi*c[8]*c[8]*c[7]
        c[57] = (11.5000 if equal(c[6] , 0.00000) else 4.00000*11.5000)
        c[58] = 0.0138542*(power(c[1]/5.40000, 1.0/2))
        c[59] = (exp(c[3]/67.3000)-1.00000)/7.00000
        c[60] = 2.00000* pi*c[8]*c[8]+2.00000* pi*c[8]*c[7]
        c[61] = c[56]*0.260000
        c[62] = c[56]*0.0600000
        c[63] = c[60]*2.00000
        c[64] = c[56]*0.680000
        c[65] = c[56]*0.0552000
        c[66] = c[56]*0.00480000
        c[67] = c[56]*0.0200000
        c[68] = c[63]/c[0]
