...
class Parameters:
    def __init__(self):
        self.Ca_TRPN_Max = 70e-3
        self.k_on = 100
        self.k_Ref_off = 0.2
        self.gamma_trpn = 2
        self.alpha_0 = 8e-3
        self.alpha_r1 = 2e-3
        self.alpha_r2 = 1.75e-3
        self.n_Rel = 3
        self.K_z = 0.15
        self.n_Hill = 3
        self.Ca_50ref = 1.05e-3
        self.z_p = 0.85
        self.beta_1 = -4
        self.beta_0 = 4.9
        self.T_ref = 56.2
        ...
        self.alpha_2 = 0.13
        self.alpha_3 = 0.625
        self.dExtensionRatiodt = 0.00000

    def to_dict(self):
        return {
            "Ca_TRPN_Max": self.Ca_TRPN_Max,
            "k_on": self.k_on,
            "k_Ref_off": self.k_Ref_off,
            "gamma_trpn": self.gamma_trpn,
            "alpha_0": self.alpha_0,
            "alpha_r1": self.alpha_r1,
            "alpha_r2": self.alpha_r2,
            "n_Rel": self.n_Rel,
            "K_z": self.K_z,
            "n_Hill": self.n_Hill,
            "Ca_50ref": self.Ca_50ref,
            "z_p": self.z_p,
            "beta_1": self.beta_1,
            "beta_0": self.beta_0,
            "T_ref": self.T_ref,
            ...
            "dExtensionRatiodt": self.dExtensionRatiodt,
        }
...
class Config:
    def __init__(
        ...
        T: int = 100,
        T_unit: float = 0.01,
        y0=[0.067593139865, 0.014417937837, 0, 0, 0],
    ):
        if param is None:
            raise ValueError("Config requires param=Parameters()")

        self.model_name = "niederer_hunter_smith_2006"
        self.curve_names = [
            "TRPN",
            "z",
            "Q_1",
        ...
        self.state_names = ['TRPN', 'z', 'Q_1', 'Q_2', 'Q_3']
        self.algebraic_names = ['Ca_b', 'Ca_i', 'beta_Tm', 'ExtensionRatio', 'lambda', 'lambda_prev', 'Ca_50', 'Ca_TRPN_50', 'alpha_Tm', 'z_max', 'overlap', 'T_Base', 'T_0', 'Q', 'Tension', 'k_off', 'J_TRPN']
        self.params = param
        self.T = T
        self.T_unit = T_unit
    ...
    def _build_constants(self):
        ...
        c[20] = p.alpha_2
        c[21] = p.alpha_3
        c[23] = p.dExtensionRatiodt

        # derived constants
        c[22] = ((c[6]*(power(c[11], c[7])))/(power(c[11], c[7])+power(c[8], c[7])))*(1.00000-(c[7]*(power(c[8], c[7])))/(power(c[11], c[7])+power(c[8], c[7])))
        c[24] = (c[6]*(power(c[11], c[7]-1.00000))*c[7]*(power(c[8], c[7])))/(power(power(c[11], c[7])+power(c[8], c[7]), 2.00000))
