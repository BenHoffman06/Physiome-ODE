...
class Config:
    def __init__(
        ...
        T: int = 100,
        T_unit: float = 0.01,
        y0=[0.1, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01, 0.01],
    ):
        if param is None:
            raise ValueError("Config requires param=Parameters()")

        self.model_name = "cortassa_aon_marban_winslow_orourke_2003"
        self.curve_names = [
            "ADP_m",
            "NADH",
            "ISOC",
        ...
        self.state_names = ['ADP_m', 'NADH', 'ISOC', 'alpha_KG', 'SCoA', 'Suc', 'FUM', 'MAL', 'OAA', 'ASP', 'Ca_m', 'delta_psi_m']
        self.algebraic_names = ['stim_period', 'Ca_i', 'ATP_m', 'ADP_i', 'NAD', 'CIT', 'V_CS', 'V_ACO', 'V_IDH', 'V_KGDH', 'V_SL', 'V_SDH', 'V_MDH', 'Ares', 'V_AAT', 'V_FH', 'delta_mu_H', 'V_C_ASP', 'V_He', 'V_O2', 'V_He_F', 'V_Hleak', 'AF1', 'V_ATPase', 'V_Hu', 'V_ANT', 'V_uni', 'V_NaCa']
        self.params = param
        self.T = T
        self.T_unit = T_unit
    ...
    def _build_constants(self):
        ...
        # derived constants
        c[105] = ((c[19]*c[20])/c[21])*log(c[78]*(power(c[16]/c[15], 0.500000)))
        c[106] = 1.00000/(1.00000+c[11]/c[58]+(power(c[11], 2.00000))/(c[58]*c[59]))+c[62]
        c[107] = power(1.00000/(1.00000+c[60]/c[11]+(c[60]*c[61])/(power(c[11], 2.00000))), 2.00000)
