...
def createLegends():
    ...
    legend_algebraic[4] = "i_Ca in component calcium_current (picoA)"
    legend_algebraic[0] = "i_K in component rapidly_activating_K_current (picoA)"
    legend_algebraic[5] = "i_K_Ca in component calcium_activated_K_current (picoA)"
    legend_algebraic[7] = "i_Na_Ca in component Na_Ca_exchanger_current (picoA)"
    ...
    legend_constants[14] = "g_K_Ca in component calcium_activated_K_current (picoS)"
    legend_constants[15] = "K_d in component calcium_activated_K_current (micromolar)"
    ...
    legend_algebraic[6] = "V_Na_Ca in component Na_Ca_exchanger_current (millivolt)"
    ...
    legend_states[3] = "Ca_ret in component ionic_concentrations (micromolar)"
    ...
    legend_rates[3] = "d/dt Ca_ret in component ionic_concentrations (micromolar)"
...
def computeRates(voi, states, constants):
    ...
    algebraic[7] = constants[16]*((power(states[2], constants[19]))/(power(constants[17], constants[19])+power(states[2], constants[19])))*(states[0]-algebraic[6])
...
class Config:
    def __init__(
        ...
        T: int = 100,
        T_unit: float = 0.01,
        y0=[-76.0, 0.1, 0.52, 0.7],
    ):
        if param is None:
            raise ValueError("Config requires param=Parameters()")

        self.model_name = "gall_susa_1999_c"
        self.curve_names = [
            "V",
            "n",
            "Ca_i",
        ...
        self.state_names = ['V', 'n', 'Ca_i', 'Ca_ret']
        self.algebraic_names = ['i_K', 'n_infinity', 'm_infinity', 'tau_n', 'i_Ca', 'i_K_Ca', 'V_Na_Ca', 'i_Na_Ca']
        self.params = param
        self.T = T
        self.T_unit = T_unit
