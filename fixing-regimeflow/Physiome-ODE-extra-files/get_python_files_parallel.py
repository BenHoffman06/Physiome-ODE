...
def process_one_model(
    ...
    model_name_raw = row_dict.get("Model Name")
    file_raw = row_dict.get("File")
    domain_raw = row_dict.get("Domain")
    ...
    try:
        ...
        if has_underconstrained:
            write_text(model_dir / "underconstrained_report.txt", stdout_text)
        ...
        record["undefined_variables"] = str(e)
        return record
...
def process_models_parallel(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in tqdm(as_completed(future_to_idx), total=len(future_to_idx), desc="Exporting CellML"):
            results.append(future.result())

    result_df = pd.DataFrame(results).sort_values("row_index").reset_index(drop=True)
    result_df.to_csv(summary_csv, index=False)
    return result_df


if __name__ == "__main__":
    result_df = process_models_parallel(
